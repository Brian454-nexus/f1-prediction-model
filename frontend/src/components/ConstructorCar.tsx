...
export const ConstructorCar: React.FC<ConstructorCarProps> = (props) => {
    const { championshipLevel, color, className, points, image, name, year = 2026, drivers, index} = props;
    const ref = useRef(null);
    const isInView = useInView(ref, { once: true });

    const driverUrl = (i: number) => championshipLevel === 'f2' ? 
    `/images/${year}/F2/${drivers[i]}.png` 
    : `/images/${year}/drivers/${drivers[i]}.png`;
    ...
    return (
        ...
        >
            <div className="flex flex-col items-center pb-40">
                ...
                <p className="uppercase tracking-sm text-xl font-bold">{name.replace('F1 Team', '')}</p>
                <div className="h-1 w-[9.6rem] border-b-[1px] border-solid border-neutral-500 mb-4 mt-4" />
                <span className="text-3xl font-display gradient-text-light">{points}</span>
                <div className={classNames(groupHoverClasses, "flex items-end relative -mt-24 z-10")}>
                    {drivers[3] && (
                        <img 
                            alt={`Driver ${drivers[3]}`} 
